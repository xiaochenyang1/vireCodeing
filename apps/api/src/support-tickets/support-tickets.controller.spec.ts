...
describe('SupportTicketsController', () => {
  ...
  it('force-takes over an admin support ticket', async () => {
    const service = {
      takeoverSupportTicket: jest.fn().mockResolvedValue({
        id: 'ticket-1',
        shipperId: 'shipper-1',
        channelName: '投诉建议',
        description: '司机沟通不及时，希望客服协助跟进',
    ...
    await expect(
      controller.takeoverSupportTicket(
        createRequest('admin-1', 'admin'),
        'ticket-1',
        body,
      ),
    ...
    expect(service.takeoverSupportTicket).toHaveBeenCalledWith(
      'admin-1',
      'ticket-1',
      {
        baseUpdatedAtIso: '2026-07-22T08:36:00.000Z',
