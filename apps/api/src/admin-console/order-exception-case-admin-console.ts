  ...
  <script>
    ...
    async function loadCases(page) {
      try {
        ...
        document.getElementById('caseList').innerHTML = result.items.length
          ? result.items.map(item => '<div class="case-row" data-case-id="' + escapeHtml(item.id) + '" onclick="loadCase(this.dataset.caseId)"><strong>' + escapeHtml(item.caseNo) + '</strong> · ' + escapeHtml(item.status) + '<div>' + escapeHtml(item.orderNo) + ' · ' + escapeHtml(item.typeLabel) + '</div><div class="muted">' + escapeHtml(item.sourceRole) + ' · ' + escapeHtml(item.createdAtIso) + '</div></div>').join('')
    ...
    async function loadCase(caseId) {
      try {
        ...
        document.getElementById('caseDetail').innerHTML = '<strong>' + escapeHtml(item.caseNo) + '</strong><p>' + escapeHtml(item.orderNo) + ' · ' + escapeHtml(item.sourceRole) + ' · ' + escapeHtml(item.status) + '</p><p>' + escapeHtml(item.typeLabel) + '：' + escapeHtml(item.description) + '</p><p>附件：' + escapeHtml((item.attachmentFileIds || []).join(', ') || '无') + '</p><p>处理结论：' + escapeHtml(item.resolutionText || '暂无') + '</p>';
        document.getElementById('caseActions').innerHTML = (item.actions || []).map(action => '<div class="action">' + escapeHtml(action.fromStatus) + ' → ' + escapeHtml(action.toStatus) + '<br>' + escapeHtml(action.content) + '<div class="muted">' + escapeHtml(action.createdAtIso) + '</div></div>').join('');
    ...
    function renderMutationButtons(status) {
      ...
      document.getElementById('caseActions').insertAdjacentHTML('beforeend', action ? '<button id="caseMutationButton" data-action="' + action + '" onclick="mutateCase(this.dataset.action)">' + labelByStatus[status] + '</button>' : '<p class="muted">工单已关闭</p>');
    }
    ...
    async function mutateCase(action) {
      ...
      const button = document.getElementById('caseMutationButton');
      if (button) button.disabled = true;
      try {
        await api('/admin/order-exception-cases/' + encodeURIComponent(selectedCaseId) + '/' + action, {
          method: 'POST',
          body: JSON.stringify({ baseUpdatedAtIso: document.getElementById('baseUpdatedAtIso').value, content }),
        });
        ...
        const nextButton = document.getElementById('caseMutationButton');
