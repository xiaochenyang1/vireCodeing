...
function createModules(
  ...
  return [
    ...
    },
    {
      ...
      stage: 'first_slice',
      summary:
        '货主修改申请后台列表、通过驳回、费用/退款/司机通知快照、审核事件审计和按订单深链的司机通知第一片已接上；留空的审核快照也会按订单金额、支付状态和司机分配自动补全；通过审核时可选填写调整后应付金额并同步改写订单 price/payable 快照，但托管补差/退款资金链路仍未自动执行。',
      metrics: [],
      pendingGaps: ['托管补差 / 差额退款资金自动执行'],
    },
    {
