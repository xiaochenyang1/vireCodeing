...
function createModules(
  ...
  return [
    ...
    },
    {
      ...
      stage: 'first_slice',
      summary:
        '货主修改申请后台列表、通过驳回、费用/退款/司机通知快照、审核事件审计和按订单深链的司机通知第一片已接上；留空的审核快照也会按订单金额、支付状态和司机分配自动补全；通过审核时可选填写调整后应付金额并同步改写订单 price/payable 快照，同时会写入资金处置结论与财务审计留痕；在线托管降价会排队部分退款 outbox，涨价会创建补差 PaymentOrder（pending，主单保持 escrowed），渠道补差支付拉起与支付单净额快照仍待后续竖切。',
      metrics: [],
      pendingGaps: [
        '补差支付渠道拉起 / 回调入账',
        '部分退款成功后支付单净额快照',
